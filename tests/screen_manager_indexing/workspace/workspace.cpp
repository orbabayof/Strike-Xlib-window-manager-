...
Window workspace::attachWorkspaceRoot()
{
	...
	// docs mention that XUnmapSubwindows is faster than XUnmapWindow for a single window
...
screen_manager::screen_manager()
{
	for (int curr_screen{0}; curr_screen < XScreenCount(dpy()); ++curr_screen)
		list.emplace_back(workspace_manager{curr_screen, settings::defualt_num_of_workspaces});
}
